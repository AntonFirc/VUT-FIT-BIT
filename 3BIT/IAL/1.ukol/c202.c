...
*/
	...
	if(s == NULL) {
		stackError(SERR_INIT);			//call error func
		return;
	}
	s->top = -1;						//top element index = -1 (stack empty)
...
*/
	...
	return (s->top <= -1);		// returns non-zero if stack empty, 0 otherwise
...
*/
	...
	return (s->top == (STACK_SIZE - 1)); //returns non-zero if stack full, 0 otherwise
...
*/
	...
	if(stackEmpty(s)) {			//checks if stack is not empty
		stackError(SERR_TOP);	//calls error func if stack is empty
	}
	else {
		*c = s->arr[s->top];		//returns value of element on top of the stack
	}
...
*/
	...
	if(!(stackEmpty(s))) {				//checks if stack is not empty
		s->arr[s->top] = 0;				//resets value of "popped" element		
		s->top = s->top - 1;			//decrements index of stack top
	}
...
*/
	...
	if(stackFull(s)){
		stackError(SERR_PUSH);			//err func called if stack full
	}
	else {
		s->top = s->top + 1;			//increments index of stack top
		s->arr[s->top] = c;				//insterts new element value
	}
