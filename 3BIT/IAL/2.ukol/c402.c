...
**/
	...
	if(*RootPtr == NULL) {						//strom je prazdny
		*RootPtr = malloc(sizeof(struct tBTNode));
		...
		while((tmp->Cont != Content)) {	//hledame jestli existuje
			if(tmp->Cont < Content) {
				if(tmp->RPtr != NULL) {		//hledame doprava
					tmp = tmp->RPtr;
				}
				else {
					break;
				...
				if(tmp->LPtr != NULL) {	//hledame doleva
					tmp = tmp->LPtr;
				}
				else {
					break;
		...
		if(tmp->Cont != Content) {		//uzel nebol najdeny
			tBTNodePtr new_leaf = malloc(sizeof(struct tBTNode));
			if(new_leaf == NULL) {
				return;
			}
			new_leaf->Cont = Content;
			new_leaf->RPtr = NULL;
			new_leaf->LPtr = NULL;
			if(new_leaf->Cont > tmp->Cont) {		//podle klice vlozi doleva nebo doprava
				tmp->RPtr = new_leaf;
			}
			else {
				tmp->LPtr = new_leaf;
...
**/
	...
	while(ptr != NULL) {	//prejdeme cely strom do najlavejsieho uzlu, uzly spracujeme
		SPushP(Stack,ptr);
		BTWorkOut(ptr);
		ptr = ptr->LPtr;
	}
...
**/
	...
	tStackP pStack;
	SInitP(&pStack);
	Leftmost_Preorder(RootPtr,&pStack);		//predeme uplne dolava
	while(!SEmptyP(&pStack)) {				//odstranime spracovane uzly
		RootPtr = STopPopP(&pStack);
		Leftmost_Preorder(RootPtr->RPtr,&pStack);	//pre kazdy uzol prejdeme uplne dolava
	}
...
**/
	...
	while(ptr != NULL) { //prejdeme do najlavejsieho uzlu, uzly nespracuvavame
		SPushP(Stack,ptr);
		ptr = ptr->LPtr;
	}
...
**/
	...
	tStackP pStack;
	SInitP(&pStack);
	Leftmost_Inorder(RootPtr,&pStack);		//prjdeme dolava
	while(!SEmptyP(&pStack)) {				//spracuvavame uzly od spodu stromu
		RootPtr = STopPopP(&pStack);
		BTWorkOut(RootPtr);
		if(RootPtr->RPtr != NULL) {				
			Leftmost_Inorder(RootPtr->RPtr,&pStack);
		}	
	}	
...
**/
	...
	while(ptr != NULL) {		//prejde do najlavejsieho uzlu, nespracovava uzly iba uklada
		SPushP(StackP,ptr);
		SPushB(StackB,true);
		ptr = ptr->LPtr;
	}
...
**/
	...
	tStackB StackB;		//inicializace zasobnikov
	SInitB(&StackB);
	tStackP StackP;
	SInitP(&StackP);

	Leftmost_Postorder(RootPtr,&StackP,&StackB);		//prejdeme uplne dolava
...
**/
	...
	if(*RootPtr == NULL) {
		...
		do {
			if(*RootPtr == NULL) {
				if(!(SEmptyP(&StackP))) {
					*RootPtr = STopPopP(&StackP);		//vyprazdnuje zasobnik
				}
			}
			else {
				if((*RootPtr)->RPtr != NULL) {
					SPushP(&StackP,(*RootPtr)->RPtr);		//ulozi prave podstromy
				}
				tBTNodePtr tmp = *RootPtr;
				*RootPtr = (*RootPtr)->LPtr;				//postupne uvolnuje
				free(tmp);
			} 
